...
export default function ActiveLobbiesPage() {
  ...
  useEffect(() => {
    ...
    socket.on("activeLobbiesUpdated", handleLobbiesUpdated);

    socket.on("connect_error", handleError);

    return () => {
      socket.off("activeLobbiesUpdated", handleLobbiesUpdated);
  ...
  return (
    <main className="min-h-screen p-6 bg-[oklch(0.06_0.007_38)] text-white">
      ...
      {loading ? (
        ...
        <div className="mt-6 text-neutral-400">No Active Lobbies</div>
      ) : (
        <div className="mt-6 space-y-4">
          {lobbies.map((lobby) => (
            ...
            >
              ...
              <p className="text-orange-500 mt-1">Code: {lobby.code}</p>

              <p className="text-sm text-neutral-400 mt-2">
                Status: {lobby.isStarted ? "Started" : "Waiting"}
              </p>
            </div>
          ))}
        </div>
